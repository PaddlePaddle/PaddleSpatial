...
class DeepWalk(paddle.nn.Layer):
    ...
    def __init__(self, config, data_feature):
        ...
        self.walk_length = config.get('walk_length', 20)
        ...
        self.txt_cache_file = './cache/evaluate_cache/embedding_{}_{}_{}.txt'.\
            format(self.model, self.dataset, self.output_dim)
        self.model_cache_file = './cache/model_cache/embedding_{}_{}_{}.m'.\
            format(self.model, self.dataset, self.output_dim)
        self.npy_cache_file = './cache/evaluate_cache/embedding_{}_{}_{}.npy'.\
            format(self.model, self.dataset, self.output_dim)
    ...
    def run(self):
        ...
        json.dump(self.ind_to_geo, open('./cache/evaluate_cache/ind_to_geo_{}.json'.format(
            self.dataset), 'w'))
        json.dump(self.geo_to_ind, open('./cache/evaluate_cache/geo_to_ind_{}.json'.format(
            self.dataset), 'w'))
