...
class LINE_SECOND(nn.Layer):
    ...
    def forward(self, I, J):
        ...
        return (vi * vj).sum(axis=-1)

    def get_embeddings(self):
        return self.node_emb.weight


class LINE(paddle.nn.Layer):
    def __init__(self, config, data_feature):
        super().__init__()
        self.order = config.get('order', 'second')
    ...
    def calculate_loss(self, batch):
        ...
        return -(F.log_sigmoid(dot_product * is_neg)).mean()
