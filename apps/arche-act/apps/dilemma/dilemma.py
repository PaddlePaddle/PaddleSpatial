...
def construct_ui(blocks, dataset: Dict[str, Dict[str, str]],
    ...
    def load_random(state):
        ...
        if len(items) > 0:
            ...
            pair_resc = "ERROR_NAME", dict(summary="ERROR_TEXT",
                                           gpt_solution="ERROR_TEXT")
        specified_task = pair_res[1]['specified_task']
        lst = list(
            (k, v) for k, v in pair_res[1].items() if k in {'summary', 'gpt_solution'})
        random.shuffle(lst)
        state = dict(name=pair_res[0], left=dict(who=lst[0][0], text=lst[0][1]),
