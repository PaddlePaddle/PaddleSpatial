...
class Ours_Trainer(object):
    def __init__(self, args):
        ...
        self.args = args
        self.root_path = '../'
        self.poi_feature_array_path = self.root_path + 'data/{task}/poi_features.npy'.format(task=self.args.task)
        self.img_feature_array_path = self.root_path + 'data/{task}/img_features.npy'.format(task=self.args.task)
    ...
    def data_prepare(self):
        poi_features = np.load(self.poi_feature_array_path)
        img_features = np.load(self.img_feature_array_path)
        label = np.load(self.label_array_path)

        poi_features = paddle.to_tensor(poi_features).cast('float32')
        img_features = paddle.to_tensor(img_features).cast('float32')
        label = paddle.to_tensor(label).cast('float32')
        return poi_features, img_features, label
    ...
    def train(self, train_id, val_id, test_id, save_model, log):
        poi_features, img_features, label = self.data_prepare()
        g = self.load_graph()
        ...
        for epoch in range(self.args.epoch_num+1):
            ########################## train ###########################
            model.train()
            pred = model(poi_features, img_features).squeeze(1)
