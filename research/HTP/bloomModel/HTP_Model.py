...
class BloomModel(BloomPreTrainedModel):
    def __init__(self, config):
        ...
        self.mha_sentence = nn.MultiHeadAttention(embed_dim=hidden_size, num_heads=self.num_head)
        self.mha_table = nn.MultiHeadAttention(embed_dim=hidden_size, num_heads=self.num_head)
        self.mha_cross = nn.MultiHeadAttention(embed_dim=hidden_size, num_heads=self.num_head)
        ...
        self.pre_content_norm = nn.LayerNorm(hidden_size, epsilon=1e-5)
        self.stable_mlp = nn.Linear(hidden_size, hidden_size)
    ...
    def forward(
        ...
        **kwargs,
        ...
        per_column_token = paddle.split(column_prompts , col, axis=1)
        per_column_virtual_num = column_prompts.shape[1] //col

        table_column_content = self.word_embeddings(semantic_table_ids)
        per_column_token = paddle.stack(per_column_token,axis=1).unsqueeze(1)
        per_column_token = paddle.tile(per_column_token, [1, row, 1, 1,1])
        per_column_token = self.pre_content_norm(per_column_token)
        table_column_content = paddle.concat([per_column_token,table_column_content], axis = 3)


        # table_embedding = table_column_content + self.position_embeddings_swpe(paddle.tile(paddle.reshape(paddle.arange(self.n_stx), [1, 1, 1, self.n_stx]), [batch, self.n_ttx_row, self.n_ttx_col, 1])) # [1,56,24,122,768]
        table_embedding = table_column_content + self.position_embeddings_swpe(paddle.tile(paddle.reshape(paddle.arange(self.n_stx+per_column_virtual_num), [1, 1, 1, self.n_stx+per_column_virtual_num]), [batch, self.n_ttx_row, self.n_ttx_col, 1])) # [1,56,24,122,768]
        ...
        seq_self_attn = self.stable_mlp(seq_self_attn)
        ...
        cross_attn = self.mha_cross(hidden_states, cross_attn_input, cross_attn_input)#[1,1024,768]
...
class BloomForCausalLM(BloomPreTrainedModel):
    ...
    def prepare_inputs_for_generation(self, input_ids, semantic_table_ids, hard_codes, use_cache=False, cache=None, **kwargs):
        # only last token for inputs_ids if cache is defined in kwargs
        attention_mask = kwargs.get("attention_mask", None)
        if cache is not None:
            input_ids = input_ids[:, -1].unsqueeze(axis=-1)

        return {"input_ids": input_ids, "semantic_table_ids":semantic_table_ids, "hard_codes":hard_codes, "attention_mask": attention_mask, "cache": cache, "use_cache": True}
